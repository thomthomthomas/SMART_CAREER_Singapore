...
class TavilyWebScraper:
    ...
    def process_skills_for_web_content(self, skills: List[str], websites_to_scrape: List[str], search_all_websites: bool = False, rank_modules: bool = True) -> Dict[str, Any]:
        ...
        for website_url in websites_to_process:
            ...
            website_entry = {
                ...
                "modules": [],
                "ranked_modules": [],  # New field for ranked modules
                "courses": []
            }
            ...
            for result in search_response.get("results", [])[:2]: # Limit to the first course
                ...
                if course_title and course_url:
                    ...
                    all_modules.extend(modules) # Collect all modules
            
            # Add all modules to the entry
            website_entry["modules"] = all_modules
            
            # Rank modules if requested and modules exist
            if rank_modules and all_modules and skills:
                logger.info(f"Ranking {len(all_modules)} modules for skill: {skills[0]}")
                ranked_modules = self._rank_modules_by_relevance(all_modules, skills[0])
                website_entry["ranked_modules"] = ranked_modules
                logger.info(f"Successfully ranked {len(ranked_modules)} modules")
    ...
    def save_results(self, data: Dict[str, Any], filename: str = "web_scraped_output.json"):
        ...
        try:
            ...
            logger.info(f"ðŸ'¾ Web scraping results saved to {filename}")
        except Exception as e:
...
def run_scraper(skills: List[str], websites: List[str], search_all: bool = False, rank_modules: bool = True, output_filename: str = "website_modules_output.json"):
    ...
    if not TAVILY_API_KEY:
        TAVILY_API_KEY = "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" # Replace with your actual key or load from config
    if not GEMINI_API_KEY:
        print("Warning: GEMINI_API_KEY not found in environment variables. Using the provided key directly.")
        GEMINI_API_KEY = "AIzaSyCKMTC6k_tl3gIOKTaYIufx9eY-6WmFJeE"
